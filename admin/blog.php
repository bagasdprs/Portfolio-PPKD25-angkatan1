...
$query = mysqli_query($conn, "SELECT blog.*, categories.nama FROM blog LEFT JOIN categories ON blog.id_category = categories.id");
$rows = mysqli_fetch_all($query, MYSQLI_ASSOC);

if (isset($_GET['delete'])) {
    $id = $_GET['delete'];

    $queryBlog = mysqli_query($conn, "SELECT * FROM blog WHERE id = '$id'");
    $rowBlog = mysqli_fetch_assoc($queryProject);
    unlink("../assets/uploads/" . $rowProject['photo']);

    $delete = mysqli_query($conn, "DELETE FROM blog WHERE id = '$id'");
    if ($delete) {
        header('location: blog.php?hapus=success');
    }
}
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="table table-responsive">
                                <a href="add_blog.php" class="btn btn-primary mb-2">Create</a>
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr>
                                                ...
                                                <td><?= $row['title'] ?></td>
                                                <td><?= $row['author'] ?></td>
                                                <td><img width="100" src="../assets/uploads/<?= $row['photo'] ?>" alt=""></td>
                                                <td>
                                                    <?php
                                                    switch ($row['status']) {
                                                        case '1':
                                                ...
                                                <td>
                                                    <a href="add_blog.php?edit=<?= $row['id'] ?>" class="btn btn-success btn-sm">Edit</a>
